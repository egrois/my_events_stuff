...
class Strategy:
    ...
    def __init__(self, df, simlog_df, event_dt, start_dt, stop_dt, opposite_stacker_off_dt, entry_stop_time_dt, time_decay_exit_start_dt, reversal_tracking_start_dt, stop_loss_tracking_start_dt, Sim, dollar_value_per_price_level, cpu_process_id=-1):
        self.df = df
	...
	self.time_decay_exit_start_loc = df.index.get_loc(time_decay_exit_start_dt)
	...
	self.stop_loss_tracking_start_loc = df.index.get_loc(stop_loss_tracking_start_dt)
...
#                print
	...
	#elif self.cur_loc >= self.opposite_stacker_off_loc and abs(self._pos) == 0 and self.isLiveStacker():
	#    self.cancelOrderStack()
        #    print "cancelled Curve |#2|", self.df.ix[self.cur_loc, 'time']
	#    self.close_position_time_dt = self.df.ix[self.cur_loc, 'time']
		...
		if available_exit_size:
                    for x in xrange(0, max_num_exit_levels):
                        ...
                        for live_order in exit_orders_authority:  #self.my_live_orders:
...
#                        print "self.my_exit_orders: ", self.my_exit_orders
	...
	if self.cur_loc < self.entry_stop_time_loc:
            ...
            #else:
            #    max_disl_ticks = 0
	    #if max_disl_ticks != self.max_disl_ticks:
		#self.max_disl_ticks = max_disl_ticks
	    if self.max_disl_ticks != 0:
	        cur_reversal = -1.0 * (self.max_disl_ticks - disl_ticks)
	        cur_percent_reversal = abs(float(cur_reversal) / float(self.max_disl_ticks))
	        #if cur_percent_reversal > self.best_percent_reversal:
		#    self.best_percent_reversal = cur_percent_reversal	
		self.best_percent_reversal = cur_percent_reversal

	    print "self.max_disl_ticks: ", self.max_disl_ticks, "disl_ticks: ", disl_ticks, "cur_percent_reversal: ", self.best_percent_reversal, self.df.ix[self.cur_loc, 'time']
...
#def compute_reversal_percentage():
#	if self.max_disl_ticks > 0:
#	    if disl_ticks < self.max_disl_ticks:
#	        cur_reversal = self.max_disl_ticks - disl_ticks
#		cur_percent_reversal = float(cur_reversal) / float(self.max_disl_ticks)
#	elif self.max_disl_ticks < 0:
#	    if disl_ticks > self.max_disl_ticks:
#		cur_reversal = 
...
#    if incl_timestamp:
#        filename = filename + "__" + cur_time_str
...
#	event_name, instrument_root, date_range_start=None, date_range_end=None, target_dates_lst=None):
    ...
    #    date_range_end_dt = datetime.strptime(date_range_end, '%Y-%m-%d')
    #elif target_dates_lst:
	#target_dates_dt_lst = [datetime.strptime(d, '%Y-%m-%d') for d in arbitr_dates_lst]

#    event_dates_times_dict = readEventDatesTimes(event_name, instrument_root)
#    dates_lst = [datetime.strptime(d, '%Y-%m-%d') for d in event_dates_times_dict]
#    dates_lst.sort()
#    sorted_dates_str_lst = [t.strftime('%Y-%m-%d') for t in dates_lst]  # all dates sorted
#
...
##    event_dates, event_dates_times_dict  = loadEventDates(event_name, instrument_root, date_range_start, date_range_end, target_dates_lst)
##    if not event_dates:
##	print "ERROR with date range.  Simulation cannot run."
##       return 0
...
#        df.set_index(df['time'], inplace=True)
	...
	symbol, df = md_dict[event_date]
        ...
        entry_stop_time_dt = stop_dt - timedelta(seconds=60)  #event_datetime_obj + timedelta(seconds=30)
	...
	simlog_df['target_exit_price'] = float('NaN')
        ...
        Sim = simulator5.Simulator(df, symbol, min_price_increment)
        Strat = Strategy(df, simlog_df, event_datetime_obj, start_dt, stop_dt, opposite_stacker_off_dt, entry_stop_time_dt, time_decay_exit_start_dt, \
			reversal_tracking_start_dt, stop_loss_tracking_start_dt, Sim, dollar_value_per_price_level, cpu_process_id)
	...
	Strat.run_trade_only = False#True  # run will terminate after conclusion of trade (pos=0)
        Strat.start()
...
#TEMP -- rewrite for paralellization    serializePnLResults(pnl_results, incl_timestamp=True, info_str=serialize_info)
    ...
    return (pnl_results, per_contract_pnl_results, trade_durations)
...
def main():
    ...
    #run_results_pnl, run_results_per_contract_pnl,  = run(50.0, -0.01, "Crude", "CL", event_dates, event_dates_times_dict, md_dict)
