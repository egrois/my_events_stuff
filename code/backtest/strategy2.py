...
#event_dates = ['2013-04-03', '2013-05-01', '2013-06-05', '2013-07-03', '2013-07-31']
#event_dates = ['2013-07-31']
...
class Strategy:
    ...
    def __init__(self, df, start_dt, max_entry_dt, stop_dt, Sim, dollar_value_per_price_level, spike_5s_pred):
        ...
        self.df = self.compute_data(df, start_dt, stop_dt) # compute additional studies and signals to be used by the strategy
    ...
    def computeMACD(self, df):
        df = tech_anal.MACD(df, 'microprice', 260, 1200, 420)  #  60, 150, 40....120, 260, 90
        df['MACDsmooth'] = pd.ewma(df['MACD'], span=15)

        return df  


    def computeBollingerBands(self, df):
        df = tech_anal.BollingerBands(df, 'microprice', 500, 2) # 120
    
        return df


    def computeRSI(self, df, n, col='close', name='RSI'):
        df['UpMoves'] = df[col] - df[col].shift(1)
        df['DoMoves'] = df[col].shift(1) - df[col]
        ...
        df[name] = map(lambda posI, negI: posI / (posI + negI), df['PosDI'], df['NegDI'])
        df[name+'_smooth'] = pd.ewma(df[name], span=20) 
        
        return df


    def computeImbalance(self, df):
	df['smooth_imbalance3'] = pd.ewma(df['imbalance3'], span=15)

	return df


    def computeStochasticOscillator(self, df, start_dt):
        df = tech_anal.FullSTO(df, 600, 60, 60, 'medium', start_dt)
    	df = tech_anal.FullSTO(df, 6000, 100, 100, 'long', start_dt)

	return df


    def computeVolumeAggr(self, df, start_event_dt, stop_dt):
        #prices = []
        #times = []
    
        volume = 0
        pre_time_loc = df.index.get_loc(start_event_dt - timedelta(seconds=1.1))
        if not np.isnan(df.ix[pre_time_loc, 'close']):
            last_aggr_close = df.ix[pre_time_loc, 'close']
        else: 
	    last_aggr_close = df.ix[pre_time_loc, 'midquote']
        for i in range(df.index.get_loc(start_event_dt - timedelta(seconds=1)), df.index.get_loc(stop_dt)):
            volume = volume + df.ix[i, 'abs_vol']
            if volume >= 200:  
                df.ix[i, 'aggr_close'] = df.ix[i, 'close']
                volume = 0
                last_aggr_close = df.ix[i, 'close']
            else:
                df.ix[i, 'aggr_close'] = last_aggr_close
        
        return df


    def computeMACDTrend(self, df):
	df['MACDtrend'] = map(lambda macd, macd_sign: MarketTrend.bullish if (macd > 0 and macd > macd_sign) else \
		(MarketTrend.weak_bullish if (macd > 0 and macd <= macd_sign) else \
		(MarketTrend.weak_bearish if (macd < 0 and macd >= macd_sign) else \
		(MarketTrend.bearish if (macd < 0 and macd < macd_sign) else \
		MarketTrend.neutral ))), df['MACD'], df['MACDsign'])
    ...
    def computeATRTrailingStop(self, df, log_df):
	...
	if self._pos > 0:
	    if df.ix[self.cur_loc, 'MACDtrend'] == MarketTrend.bullish:
	    ...
	    elif df.ix[self.cur_loc, 'MACDtrend'] == MarketTrend.weak_bullish:
		log_df.ix[self.cur_loc, 'atr_stop_bot'] = max(log_df.ix[self.cur_loc-1, 'atr_stop_bot'], df.ix[self.cur_loc, 'top_bid_price'] - 1 * min_price_increment)
		log_df.ix[self.cur_loc, 'atr_stop_top'] = df.ix[self.cur_loc, 'atr_high_bound']
        elif self._pos < 0:
	    if df.ix[self.cur_loc, 'MACDtrend'] == MarketTrend.bearish:
		#print "inside MACD bearish branch", log_df.ix[self.cur_loc-1, 'atr_stop_top'], df.ix[self.cur_loc, 'atr_high_bound'], str(df.ix[self.cur_loc, 'time'])
	    ...
	    elif  df.ix[self.cur_loc, 'MACDtrend'] == MarketTrend.weak_bearish:
		log_df.ix[self.cur_loc, 'atr_stop_top'] = min(log_df.ix[self.cur_loc-1, 'atr_stop_top'], df.ix[self.cur_loc, 'top_ask_price'] + 1 * min_price_increment)
		log_df.ix[self.cur_loc, 'atr_stop_bot'] = df.ix[self.cur_loc, 'atr_low_bound']
    ...
    def compute_data(self, df, start_dt, stop_dt):
        df = self.computeEMAs(df)
	df = self.computeVolumeAggr(df, start_dt, stop_dt)
        df = self.computeMACD(df)
	df = self.computeATR(df, 100)
	df = self.computeBollingerBands(df)
	df = self.computeRSI(df, 50, col='close', name='RSI')
	df = self.computeRSI(df, 100, col='close', name='RSI_long')
	df = self.computeImbalance(df)
	df = self.computeStochasticOscillator(df, start_dt)
        df = self.computeMACDTrend(df)
    ...
    def sendBuyEntryPassive(self):
	new_order = {'type': 'entry_passive', 'direction': 'buy', 'size': 50, 'levels_to_cross': 0, 'order_time_loc': self.cur_loc, 'ttl': 1}
    ...
    def sendSellEntryPassive(self):
	new_order = {'type': 'entry_passive', 'direction': 'sell', 'size': 50, 'levels_to_cross': 0, 'order_time_loc': self.cur_loc, 'ttl': 1}
    ...
    def sendBuyEntry(self):
        new_order = {'type': 'entry', 'direction': 'buy', 'size': 50, 'levels_to_cross': 2, 'order_time_loc': self.cur_loc, 'ttl': 0}
    ...
    def sendSellEntry(self):
        new_order = {'type': 'entry', 'direction': 'sell', 'size': 50, 'levels_to_cross': 2, 'order_time_loc': self.cur_loc, 'ttl': 0}
	...
	if self._pos == 0 and self.cur_loc < self.max_entry_loc and self.cur_loc < self.time_offset_loc(5):
	    #print "basic buy ok"
	    if ((self.spike_5s_pred > 0 and self.df.ix[self.cur_loc, 'smooth_imbalance3'] <= 0.2) or (self.spike_5s_pred < 0 and self.df.ix[self.cur_loc, 'smooth_imbalance3'] >= 0.2)):
		#print "imbalance ok"
		if self.df.ix[self.cur_loc, 'RSI'] > 0.40 and self.df.ix[self.cur_loc, 'RSI'] <= 0.95:
		    #print "RSI ok"
		    if self.df.ix[self.cur_loc, 'SO%k_medium'] > self.df.ix[self.cur_loc, 'SO%d_medium']:
			"""self.df.ix[self.cur_loc, 'SO%k_medium'] > 0.5 and self.df.ix[self.cur_loc, 'SO%k_medium'] <= 0.95 and"""
			#print "Stochastic ok"
	    		return True
	#print "no buy!"
	return False
		 
#		and self.df.ix[self.cur_loc, 'RSI'] <= 0.7 \
#		and self.df.ix[self.cur_loc, 'MACDtrend'] == MarketTrend.bullish and self.df.ix[self.cur_loc, 'aggr_close'] >= self.df.ix[self.cur_loc, 'bb_upper']:
#	    #if self.df.ix[self.cur_loc, 'RSI'] > 0.7:
#	    #	print "overbought, DON'T BUY    RSI=", self.df.ix[self.cur_loc, 'RSI']
#	    print "RSI=", self.df.ix[self.cur_loc, 'RSI'], "RSI_long=", self.df.ix[self.cur_loc, 'RSI_long']
#	    return True
#	else:
#	    return False 

	#	and self.df.ix[self.cur_loc, 'MACD'] > 0 and self.df.ix[self.cur_loc, 'MACD'] > self.df.ix[self.cur_loc, 'MACDsign'] \
	#	and self.df.ix[self.cur_loc-1, 'aggr_close'] < self.df.ix[self.cur_loc-1, 'microprice_ema_26000ms'] and self.df.ix[self.cur_loc, 'aggr_close'] > self.df.ix[self.cur_loc, 'microprice_ema_26000ms']:
	#    return True
	#else:
	#    return False
	...
	if self._pos == 0 and self.cur_loc < self.max_entry_loc and self.cur_loc < self.time_offset_loc(5):
	    ...
	    if ((self.spike_5s_pred < 0 and self.df.ix[self.cur_loc, 'smooth_imbalance3'] <= -0.2) or (self.spike_5s_pred > 0 and self.df.ix[self.cur_loc, 'smooth_imbalance3'] <= -0.2)):
		#print "imbalance ok"
		if self.df.ix[self.cur_loc, 'RSI'] < 0.60 and self.df.ix[self.cur_loc, 'RSI'] >= 0.05:
		    #print "RSI ok"
		    if self.df.ix[self.cur_loc, 'SO%k_medium'] < self.df.ix[self.cur_loc, 'SO%d_medium']:
			"""self.df.ix[self.cur_loc, 'SO%k_medium'] < 0.5 and self.df.ix[self.cur_loc, 'SO%k_medium'] >= 0.05 and"""
			#print "Stochastic ok"
	    		return True
	#print "no sell"
	return False		

#		and self.df.ix[self.cur_loc, 'RSI'] >= 0.3 \
#		and self.df.ix[self.cur_loc, 'MACDtrend'] == MarketTrend.bearish and self.df.ix[self.cur_loc, 'aggr_close'] <= self.df.ix[self.cur_loc, 'bb_lower']: 
#	    #if self.df.ix[self.cur_loc, 'RSI'] < 0.3:
#	    #	print "oversold, DON'T SELL    RSI=", self.df.ix[self.cur_loc, 'RSI']
#	    print "RSI", self.df.ix[self.cur_loc, 'RSI'], "RSI_long=", self.df.ix[self.cur_loc, 'RSI_long']
#	    return True
#        else:
#            return False
	...
	if self._pos < 0 \
	        and (self.df.ix[self.cur_loc, 'MACDtrend'] == MarketTrend.weak_bearish or self.df.ix[self.cur_loc, 'aggr_close'] >= self.log_df.ix[self.cur_loc, 'atr_stop_top']):
	    return True
        else:
            return False	

	#	and ((self.df.ix[self.cur_loc-1, 'MACD'] < self.df.ix[self.cur_loc-1, 'MACDsign'] and self.df.ix[self.cur_loc, 'MACD'] > self.df.ix[self.cur_loc, 'MACDsign']) \
	#		or (self.df.ix[self.cur_loc-1, 'aggr_close'] < self.df.ix[self.cur_loc-1, 'microprice_ema_26000ms'] and self.df.ix[self.cur_loc, 'aggr_close'] > self.df.ix[self.cur_loc, 'microprice_ema_26000ms'])):
	#    return True
	#else:
	#    return False
	...
	if self._pos > 0 \
		and (self.df.ix[self.cur_loc, 'MACDtrend'] == MarketTrend.weak_bullish or self.df.ix[self.cur_loc, 'aggr_close'] <= self.log_df.ix[self.cur_loc, 'atr_stop_bot']):
            return True
        else:
            return False
    ...
    def onTick(self):
        ...
        if self.df.ix[self.cur_loc, 'time'] >= self.start_dt + timedelta(seconds=5):
            self.df, self.log_df = self.computeATRTrailingStop(self.df, self.log_df)
	...
	new_order = None
		...
		if self.df.ix[self.cur_loc, 'MACD'] > 0 and self.df.ix[self.cur_loc, 'MACD'] > self.df.ix[self.cur_loc, 'MACDsign']:
		    print "MACD bullish", self.df.ix[self.cur_loc, 'MACD']
		elif self.df.ix[self.cur_loc, 'MACD'] < 0 and self.df.ix[self.cur_loc, 'MACD'] < self.df.ix[self.cur_loc, 'MACDsign']:
		    print "MACD bearish", self.df.ix[self.cur_loc, 'MACD']
	    else:
